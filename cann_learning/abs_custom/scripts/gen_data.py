...
#
# [MODIFY] 创建新算子时修改:
# 1. total_length: 总元素数（需与 abs_custom.asc 中的保持一致）
# 2. dtype: 数据类型（需与 abs_custom.asc 中的保持一致）
...
# [MODIFY] 数据规格参数
total_length = 8192 * 8    # 总元素数，需与 abs_custom.asc 中 main 函数保持一致
dtype = np.float16         # 数据类型，需与 abs_custom.asc 中模板参数保持一致

# [MODIFY] 生成输入数据（FP16，含正负随机值）

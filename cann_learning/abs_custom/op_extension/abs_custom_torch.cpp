...
#include "../op_kernel/abs_custom_tiling.h"

// [MODIFY] extern 声明 kernel 入口 - abs_custom → <your_op>，保持 _kernel 后缀
// 从 abs_custom_kernel.asc 中的核函数签名复制，去掉 __global__ __vector__，加 extern "C"
extern "C" void abs_custom_kernel(uint32_t blockDim, void *l2Ctrl, aclrtStream stream,
                                   uint8_t *x, uint8_t *y, uint8_t *tiling);

// [MODIFY] 算子 PyTorch 实现 - abs_custom → <your_op>，保持 _torch 后缀
namespace ascend_kernel {

at::Tensor abs_custom_torch(const at::Tensor& x)
{
    TORCH_CHECK(x.scalar_type() == at::kHalf, "only FP16 supported");
    ...
    // [MODIFY] 调用 kernel - abs_custom → <your_op>，保持 _kernel 后缀
    abs_custom_kernel(blockNum, nullptr, aclStream,
